...
char *nom,*mdp,*code,*mode,*selection,*storage_dir;
...
tamb(selection);
strcpy(storage_dir,CIRRUS_CONTAIN_DIR);
			...
			if(db_ntuples(result)==0)
         	...
         	if(success==1)
         		...
         		if(strcmp(selection,"")!=0) 
         			...
         			if(db_ntuples(result)>0) 
         				{
         				db_getvalue(result,0,0,reponse,200);
         				sprintf(query,"%s%s",storage_dir,reponse);
         				unlink(query);
         				sprintf(query, "delete from entree where owner=\'%s\' and id=\'%s\';",nom,selection);
         				db_query(handler,query);
         				}
         			else
         				{
...
<div data-role=\"page\" data-theme=\"b\">\n\
      ...
      <div data-role=\"navbar\">\n\
            <ul>\n\
               <li>\n\
               <form action=\"/cgi-bin/cirrus_pagesweb.cgi\" method=\"post\" id=\"form-add\" data-transition=\"none\" data-rel=\"dialog\">\n\
