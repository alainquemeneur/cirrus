	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
      		...
      		if(succes==1)
      			{
					strcpy(query,"select nom from compte where nom!=\'Admin\' and actif=\'non\' order by nom asc;");
					db_query(handler,query);
					}
...
if(succes==0) printf("%s\n",reponse);
	...
	{
	printf("<p style=\"text-align:center\"><strong>Gestion des comptes</strong></p>\n");
	if(db_ntuples(result)>0)
		{
		printf("<form autocomplete=\"off\" action=\"/cgi-bin/cirrus_activer.cgi\" method=\"post\" id=\"form-activer\" data-transition=\"none\" data-rel=\"dialog\">\n\
                  <input type=\"hidden\" name=\"txt-password\" id=\"txt-password\" value=\"%s\">\n\
                  <input type=\"hidden\" name=\"txt-code\" id=\"txt-code\" value=\"%s\">\n\
                  <fieldset data-role=\"controlgroup\">\n\
               	<legend>Comptes non activés</legend>\n",mdp,code);
		...
		printf("</fieldset>\n<input type=\"submit\" data-mini=\"false\" data-inline=\"false\" data-theme=\"c\" data-icon=\"plus\" value=\"Activer ce compte\">\n</form>\n<hr>\n");
		}
	db_clear_result(result);
	...
	if(db_ntuples(result)>0)
		{
		printf("<form autocomplete=\"off\" action=\"/cgi-bin/cirrus_desactiver.cgi\" method=\"post\" id=\"form-desactiver\" data-transition=\"none\" data-rel=\"dialog\">\n\
                  <input type=\"hidden\" name=\"txt-password\" id=\"txt-password\" value=\"%s\">\n\
                  <input type=\"hidden\" name=\"txt-code\" id=\"txt-code\" value=\"%s\">\n\
                  <fieldset data-role=\"controlgroup\">\n\
               	<legend>Comptes actifs</legend>\n",mdp,code);
		for(i=0;i<db_ntuples(result);i++)
			{
			db_getvalue(result,i,0,reponse,200);
			if(i==0) printf("<input type=\"radio\" name=\"radio-choice-a\" id=\"radio-choice-a%d\" value=\"%s\" data-theme=\"c\" checked=\"checked\">\n\
               <label for=\"radio-choice-a%d\">%s</label>\n",i,reponse,i,reponse);
			else printf("<input type=\"radio\" name=\"radio-choice-a\" id=\"radio-choice-a%d\" value=\"%s\" data-theme=\"c\">\n\
               <label for=\"radio-choice-a%d\">%s</label>\n",i,reponse,i,reponse);
			}
		printf("</fieldset>\n<input type=\"submit\" data-mini=\"false\" data-inline=\"false\" data-theme=\"c\" data-icon=\"minus\" value=\"Désactiver ce compte\"></form>\n");
		}
	db_clear_result(result);
	db_close(handler);
	}
